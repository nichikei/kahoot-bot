...
class Kahoot:
	...
	def startGame(self):
		try:
			...
			if self.DEBUG:
				import traceback
				print(f"[DEBUG] startGame error: {type(e).__name__}: {e}")
				traceback.print_exc()
			raise
	...
	async def _play(self):
		...
		if self.DEBUG:
			print(f"[DEBUG] WebSocket URL: {url}")
		try:
			...
			quiz_counter = 0   # đếm số câu quiz thật đã gặp
			total_counter = 0  # đếm tổng số câu (kể cả survey)
			tFADone = 0
			if self.quizID:
				self.answers = await self.findAnswers()
				if self.answers:
					print(f'✅ Đã tải đáp án! Đang chờ host bắt đầu game...')
			async for rawMessage in client:
				...
				if 'id' in message:
					...
					if self.DEBUG and kind:
						print(f"[DEBUG] >> {kind} (id={message['id']})")
					...
					if kind == 'RESET_TWO_FACTOR_AUTH' and not tFADone:
						...
						if kind not in ('GET_READY', 'START_QUESTION', 'TIME_UP',
						                'ANSWER_RESPONSE', 'REVEAL_RANKING', 'REVEAL_ANSWER'):
							print(kind.replace('_', ' '))
					if kind == 'USERNAME_ACCEPTED':
						print(f'✅ Đã vào lobby với nickname: {self.nickname}')
					elif kind == 'USERNAME_REJECTED':
						self.error('Duplicate name')
					elif kind == 'START_QUIZ':
						print('🚀 Game bắt đầu!')
						if self.DEBUG:
							print(f"[DEBUG] START_QUIZ keys: {list(data.keys())}")
							print(f"[DEBUG] START_QUIZ data: {json.dumps(data, indent=2, default=str)}")
						# Thử lấy quizId từ START_QUIZ data
						if not self.quizID:
							for key in ('quizId', 'quiz_id', 'quizUUID', 'quizID', 'kahootId', 'uuid'):
								if key in data and data[key]:
									self.quizID = data[key]
									print(f"🆔 Quiz ID từ game: {self.quizID}")
									break
						# Thử lấy quiz name để search
						if not self.quizID and not self.quizName:
							for key in ('quizTitle', 'nameOfQuiz', 'title', 'name', 'quizName'):
								if key in data and data[key]:
									self.quizName = data[key]
									print(f"📝 Quiz name từ game: {self.quizName}")
									break
						quizAnswers = data.get('quizQuestionAnswers', None)
						if not self.answers:
							self.answers = await self.findAnswers(accepted_answers=quizAnswers)
						if self.answers:
							print(f'✅ Đã tải đáp án! ({len(self.answers)} câu)')
						else:
							print('⚠️  Không tìm được đáp án - sẽ dùng đáp án mặc định')
					elif kind == 'START_QUESTION':
						total_counter += 1
						# Dùng questionIndex từ data nếu có, không thì dùng counter
						qIndex = data.get('questionIndex', None)
						qType = data.get('type', '')
						display_num = (qIndex + 1) if qIndex is not None else total_counter

						if qType not in allowedTypes:
							print(f'\n⏭️  Câu {display_num} [{qType}]: Bỏ qua (survey/slide)')
							continue

						# Câu quiz thật: dùng quiz_counter làm index vào answers
						ans_idx = quiz_counter
						quiz_counter += 1

						print(f'\n❓ Câu {display_num}:', end=' ')
						if self.answers and ans_idx < len(self.answers):
							correct = self.answers[ans_idx]['index']
							ans_text = self.answers[ans_idx].get('answer', '')
							print(f'→ {ans_text} [{self.colors[int(correct)]}]')
						else:
							correct = DEFAULT_ANSWER
							print(f'→ Không có đáp án, chọn mặc định ({correct})')
						await self.sendAnswer(int(correct))
					elif kind == 'ANSWER_RESPONSE':
						if self.DEBUG:
							print(data)
					elif kind == 'TIME_UP':
						pass
					elif kind == 'REVEAL_RANKING':
						if self.DEBUG:
							print(data)
					elif kind == 'RESET_CONTROLLER':
						print('⚠️  Bị ngắt kết nối bởi server.')
						self.gracefulExit()
					elif kind == 'GAME_OVER':
						print('\n🏆 Game kết thúc!')
						self.gracefulExit()
			...
			if self.DEBUG:
				import traceback
				print(f"[DEBUG] _play error: {type(e).__name__}: {e}")
				traceback.print_exc()
			raise
	...
	def checkPin(self):
		...
		data = resp.json()
		if self.DEBUG:
			print(f"[DEBUG] Session keys: {list(data.keys())}")
			print(f"[DEBUG] Session data: {json.dumps({k:v for k,v in data.items() if k != 'challenge'}, indent=2, default=str)}")
		# Lưu liveGameId
		self.liveGameId = data.get('liveGameId', None)
		# Tự động lấy Quiz ID từ session - thử nhiều field name
		if not self.quizID:
			for key in ('quizId', 'quiz_id', 'quizUUID', 'quizID', 'kahootId'):
				if key in data and data[key]:
					self.quizID = data[key]
					print(f"🆔 Quiz ID tự động: {self.quizID}")
					break
		# Thử lấy quizId qua liveGameId API
		if not self.quizID and self.liveGameId:
			self._tryGetQuizFromLiveGame()
		self.sessionID = self.solveChallenge(data["challenge"])

	def _tryGetQuizFromLiveGame(self):
		"""Thử lấy quiz ID từ các API endpoint khác nhau"""
		try_urls = [
			f"https://play.kahoot.it/rest/games/{self.liveGameId}",
			f"https://play.kahoot.it/rest/challenges/pin/{self.pin}",
		]
		for url in try_urls:
			try:
				...
				if resp.status_code == 200:
					data = resp.json()
					if self.DEBUG:
						print(f"[DEBUG] {url} → {json.dumps(data, indent=2, default=str)[:500]}")
					for key in ('quizId', 'kahootId', 'uuid', 'quizUUID', 'quiz_id'):
						if key in data and data[key]:
							self.quizID = data[key]
							print(f"🆔 Quiz ID tìm thấy: {self.quizID}")
							return
					# Thử nested
					if 'kahoot' in data and isinstance(data['kahoot'], dict):
						for key in ('uuid', 'quizId', 'id'):
							if key in data['kahoot'] and data['kahoot'][key]:
								self.quizID = data['kahoot'][key]
								print(f"🆔 Quiz ID tìm thấy: {self.quizID}")
								return
			except Exception:
				pass
